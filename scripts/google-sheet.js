...
let noCover = "https://nng-phinf.pstatic.net/MjAyNDAzMjhfMTI1/MDAxNzExNjM1MTI3NDU0.HMTlIhAvRlQdAnu7UOaS-B_9tmO7JYIBzn_g0OROKK4g.bZdEnGnyoxdjuBXC7Bistbd8-dO6tOrClKOy-LlUZzkg.JPEG/006.jpg?type=f174_174";
...
google.charts.load("current", { packages: ["corechart"] }).then(() => {
    ...
    query.send((response) => {
        ...
        query2.send(handleSecondSheetResponse);
    });
});

function handleSecondSheetResponse(response) {
    if (response.isError()) {
    ...
    var profileImageUrl = data.getValue(25, 1); // B26 셀의 값 (26행, 2열)
...
function random_select(jsonObj, num) {
    ...
    for (i; i < num; i = i + 1) {
        ...
        else if (musiclist[rnd].cover_link == "No thumbnail found") coverImg.src = noCover;
